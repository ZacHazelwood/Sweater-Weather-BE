class YelpFacade
  ...
  def self.create_yelp_results(params)
    ...
    yelp_search = YelpService.get_business_data(location, params[:term])
    business = { name: yelp_search[:businesses][0][:name], address: yelp_search[:businesses][0][:location][:display_address] }
